...
def draw_L(window, circle, r, c):
    ...
    for k in range(r+3):
        ...
        x = original_x
        y = y + 2 * radius
    start_for_bottom_x = original_x + 6*radius
    start_for_bottom_y = original_y + (2 * radius * (r))

    x = start_for_bottom_x
    y = start_for_bottom_y
    for k in range(3):
        for j in range(c):
            new_circle = rg.Circle(rg.Point(x,y),radius)
            new_circle.fill_color = circle.fill_color
...
def draw_wall_on_right(rectangle, n, window):
    ...
    original_top_left = rectangle.corner_1
    original_bottom_right = rectangle.corner_2

    ogtl_x = rectangle.corner_1.x
    ogtl_y = rectangle.corner_1.y

    ogbr_x = rectangle.corner_2.x
    ogbr_y = rectangle.corner_2.y

    for k in range(n):
        for j in range(k+1):
            new_rect = rg.Rectangle(rg.Point(ogtl_x, ogtl_y), rg.Point(
                ogbr_x, ogbr_y))
            new_rect.attach_to(window)
            window.render(0.1)
            ogtl_x = ogtl_x - rectangle.get_width()
            ogbr_x = ogbr_x - rectangle.get_width()
        ogtl_x = original_top_left.x
        ogbr_x = original_bottom_right.x

        ogtl_y = ogtl_y + rectangle.get_height()
        ogbr_y = ogbr_y + rectangle.get_height()
